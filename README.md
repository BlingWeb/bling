...
or the `bling` command line, instead of you clicking through it by hand.

That automation matters because hostile pages fight back. A malware page will bounce a visitor
through a chain of redirects and **cloak** itself based on who's asking: it serves the real
...
cd bling
pip install -e .
playwright install chrome      # bling drives channel="chrome", real Google Chrome rather than Chromium
```
...
works the same. Use your own account rather than a shared one; the login ties to a local
Chrome profile, and two people on one account collide with an "another host joined" error.
...
For an attended session, such as signing into a site by hand inside the VM, keep it open:
...
To see a page the way another country does, and spot **geo-cloaked** malware that serves
...
- **[`examples/`](examples/README.md)**: seven runnable walkthroughs, from the
  [quickstart](examples/quickstart.py) and HAR [URL triage](examples/url_triage.py) to
...
it's a list of the exact malware URLs the page touched. Read it *as data*, in a text or JSON
editor, `bling urls`, or the [URL-triage example](examples/url_triage.py), and **don't open a captured HAR
...
log in, so this rarely bites; still, know it's there.
